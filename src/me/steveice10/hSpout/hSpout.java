...
public class hSpout extends CommonPlugin {
	...
	public void onEnable() {
		try {
			...
			Spout.getGame().getLogger().severe("[hSpout] Failed to init etc class!");
			e.printStackTrace();
			return;
		}
		...
		try {
			...
			this.getGame().getLogger().severe("[hSpout] Failed to load hSpout plugins!");
			e.printStackTrace();
		}
	...
	public void loadConfig() {
		File configFile = new File(this.getDataFolder(), "hSpout.yml");
		...
		if(!configFile.exists()) {
			try {
				...
				this.getGame().getLogger().severe("[hSpout] Failed to create hSpout.yml!");
				e.printStackTrace();
				return;
			}
